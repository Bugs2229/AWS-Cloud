############################################
# Locals (naming convention: UltraM-*)
############################################
locals {
...
resource "aws_internet_gateway" "ultram_igw01" {
...
# Explanation: Ultram wants the private base to call home—EIP gives the NAT a stable “holonet address.”
resource "aws_eip" "ultram_igw" {
  domain = "vpc"
...
resource "aws_nat_gateway" "ultram_nat01" {
  allocation_id = aws_eip.ultram_nat_eip01.id
  ...
  depends_on = [aws_internet_gateway.ultram_igw01]
}
...
resource "aws_route_table" "ultram_public_rt01" {
  vpc_id = aws_vpc.ultram_vpc_01.id
...
resource "aws_route" "ultram_public_default_route" {
  route_table_id         = aws_route_table.ultram_public_rt01.id
  destination_cidr_block = "0.0.0.0/0"
  gateway_id             = aws_internet_gateway.ultram_igw01.id
}

# Explanation: Attach public subnets to the “public lanes.”
resource "aws_route_table_association" "ultram_public_rta" {
  count          = length(aws_subnet.ultram_public_subnets)
  subnet_id      = aws_subnet.ultram_public_subnets[count.index].id
  route_table_id = aws_route_table.ultram_public_rt01.id
}

# Explanation: Private route table = “stay hidden, but still ship supplies.”
resource "aws_route_table" "ultram_private_rt01" {
  vpc_id = aws_vpc.ultram_vpc_01.id
...
resource "aws_route" "ultram_private_default_route" {
  route_table_id         = aws_route_table.ultram_private_rt01.id
  destination_cidr_block = "0.0.0.0/0"
  nat_gateway_id         = aws_nat_gateway.ultram_nat01.id
}

# Explanation: Attach private subnets to the “stealth lanes.”
resource "aws_route_table_association" "ultram_private_rta" {
  count          = length(aws_subnet.ultram_private_subnets)
  subnet_id      = aws_subnet.ultram_private_subnets[count.index].id
  route_table_id = aws_route_table.ultram_private_rt01.id
}
...
resource "aws_security_group" "ultram_ec2_sg01" {
...
resource "aws_security_group" "ultram_rds_sg01" {
  name        = "${local.name_prefix}-rds-sg01"
  description = "RDS security group"
  vpc_id      = aws_vpc.ultram_vpc_01.id

  # TODO: student adds inbound MySQL 3306 from aws_security_group.ultram_ec2_sg01.id
...
resource "aws_db_subnet_group" "ultram_rds_subnet_group01" {
  name       = "${local.name_prefix}-rds-subnet-group01"
  subnet_ids = aws_subnet.ultram_private_subnets[*].id
...
resource "aws_db_instance" "lab-mysql" {
  ...
  db_subnet_group_name   = aws_db_subnet_group.ultram_rds_subnet_group01.name
  vpc_security_group_ids = [aws_security_group.ultram_rds_sg01.id]
...
resource "aws_iam_role" "ultram_ec2_role01" {
...
resource "aws_iam_role_policy_attachment" "ultram_ec2_ssm_attach" {
  role       = aws_iam_role.ultram_ec2_role01.name
  policy_arn  = "arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore"
}

# Explanation: EC2 must read secrets/params during recovery—give it access (students should scope it down).
resource "aws_iam_role_policy_attachment" "ultram_ec2_secrets_attach" {
  role      = aws_iam_role.ultram_ec2_role01.name
...
resource "aws_iam_role_policy_attachment" "ultram_ec2_cw_attach" {
  role      = aws_iam_role.ultram_ec2_role01.name
  policy_arn = "arn:aws:iam::aws:policy/CloudWatchAgentServerPolicy"
}

# Explanation: Instance profile is the harness that straps the role onto the EC2 like bandolier ammo.
resource "aws_iam_instance_profile" "ultram_instance_profile01" {
...
resource "aws_instance" "ultram_ec201" {
  ami                    = var.ec2_ami_id
  instance_type           = var.ec2_instance_type
  subnet_id               = aws_subnet.ultram_public_subnets[0].id
  vpc_security_group_ids  = [aws_security_group.ultram_ec2_sg01.id]
  iam_instance_profile    = aws_iam_instance_profile.ultram_instance_profile01.name
...
resource "aws_ssm_parameter" "ultram_db_endpoint_param" {
  name  = "/lab/db/endpoint"
  type  = "String"
  value = aws_db_instance.ultram_rds01.address
...
resource "aws_ssm_parameter" "ultram_db_port_param" {
  name  = "/lab/db/port"
  type  = "String"
  value = tostring(aws_db_instance.ultram_rds01.port)
...
resource "aws_ssm_parameter" "ultram_db_name_param" {
  name  = "/lab/db/name"
  type  = "String"
  value = var.db_name
...
resource "aws_secretsmanager_secret" "ultram_db_secret01" {
...
resource "aws_secretsmanager_secret_version" "ultram_db_secret_version01" {
  secret_id = aws_secretsmanager_secret.ultram_db_secret01.id

  secret_string = jsonencode({
    username = var.db_username
    password = var.db_password
    host     = aws_db_instance.ultram_rds01.address
    port     = aws_db_instance.ultram_rds01.port
    dbname   = var.db_name
  })
}
...
resource "aws_cloudwatch_log_group" "ultram_log_group01" {
...
resource "aws_cloudwatch_metric_alarm" "ultram_db_alarm01" {
  ...
  alarm_actions       = [aws_sns_topic.ultram_sns_topic01.arn]
...
resource "aws_sns_topic" "ultram_sns_topic01" {
...
resource "aws_sns_topic_subscription" "ultram_sns_sub01" {
  topic_arn = aws_sns_topic.ultram_sns_topic01.arn
  protocol  = "email"
